...
- **Post insights as PR comments**, check runs, or annotations
...
| `json_output` | boolean | No | Request structured JSON response |
| `on_cel` | string | No | CEL expression for conditional triggering |
| `output` | string | Yes | Output destination: `pr-comment`, `check-run`, or `annotation` |
...
Creates a GitHub check run with the analysis:
...
- ✅ Appears in PR checks section
- ✅ Formal status reporting
- ✅ Integrates with branch protection

### Annotation

Adds analysis as PipelineRun annotations:
...
- ✅ Stored with the pipeline run
- ⚠️ Size limitations apply
