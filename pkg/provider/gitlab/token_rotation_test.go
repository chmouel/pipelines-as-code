		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				...
				if tt.wantErrContains != "" {
					assert.Assert(t, err != nil && containsString(err.Error(), tt.wantErrContains),
						"error %q should contain %q", err, tt.wantErrContains)
				}
				return
			}
...
func TestMaybeRotateTokenSecretUpdateFails(t *testing.T) {
	...
	assert.Assert(t, containsString(err.Error(), "secret update failed"),
		"error should mention secret update failure: %v", err)
