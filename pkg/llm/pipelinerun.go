...
func analysisPipelineRunName(parentName, roleName string) string {
	return hashedPipelineRunName(parentName, roleName, "llm", fmt.Sprintf("%s-%s", parentName, roleName))
}

// hashedPipelineRunName builds a Kubernetes-safe name with an 8-char hash suffix.
// hashInput controls the hash to preserve backward-compatible name generation.
func hashedPipelineRunName(parentName, roleName, infix, hashInput string) string {
	...
	name := fmt.Sprintf("%s-%s-%s-%s", prefix, infix, role, suffix)
	if len(name) <= 63 {
		return name
	}
...
func cloneURL(event *info.Event) string {
	if event.CloneURL != "" {
		return event.CloneURL
	}
	return event.URL
}
