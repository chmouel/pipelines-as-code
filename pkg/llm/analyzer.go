...
func (a *Analyzer) Analyze(ctx context.Context, request *AnalyzeRequest) ([]AnalysisResult, error) {
	...
	for _, role := range config.Roles {
		...
		if analysisRequest.MaxTokens == 0 {
			analysisRequest.MaxTokens = DefaultConfig.MaxTokens
		}
		if analysisRequest.TimeoutSeconds == 0 {
			analysisRequest.TimeoutSeconds = DefaultConfig.TimeoutSeconds
		}
		...
		roleLogger.With(
			...
			"json_parsed", response.JSONParsed != nil,
...
func (a *Analyzer) shouldTriggerRole(role v1alpha1.AnalysisRole, celContext map[string]interface{}) (bool, error) {
	...
	result, err := cel.Value(role.OnCEL, celContext["body"], 
		...
		make(map[string]interface{})) // files - empty for pipeline context
	if err != nil {
...
func (a *Analyzer) validateConfig(config *v1alpha1.AIAnalysisConfig) error {
	...
	for i, role := range config.Roles {
		...
		if role.Output == "" {
			return fmt.Errorf("role[%d]: output destination is required", i)
		}

		// Validate output destination
		switch role.Output {
		case "pr-comment", "check-run", "annotation":
			// Valid output destinations
		default:
			return fmt.Errorf("role[%d]: invalid output destination '%s'", i, role.Output)
		}
	}
...
func (a *Analyzer) createClient(ctx context.Context, config *v1alpha1.AIAnalysisConfig, namespace string) (Client, error) {
	clientConfig := &ClientConfig{
		Provider:       LLMProvider(config.Provider),
		TokenSecretRef: &secrets.Secret{
			Name: config.TokenSecretRef.Name,
			Key:  config.TokenSecretRef.Key,
		},
		TimeoutSeconds: config.TimeoutSeconds,
		MaxTokens:      config.MaxTokens,
	}
...
func (a *Analyzer) GetSupportedProviders() []LLMProvider {
