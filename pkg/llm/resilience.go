...
func (cb *CircuitBreaker) Execute(ctx context.Context, fn func() (*AnalysisResponse, error)) (*AnalysisResponse, error) {
	if !cb.canExecute() {
...
func NewResilientClient(client Client, logger *zap.SugaredLogger) *ResilientClient {
...
func NewResilientClientWithConfig(client Client, retryConfig *RetryConfig, cbFailureThreshold int, cbTimeout time.Duration, logger *zap.SugaredLogger) *ResilientClient {
	return &ResilientClient{
		client:         client,
...
func (rc *ResilientClient) Analyze(ctx context.Context, request *AnalysisRequest) (*AnalysisResponse, error) {
	var lastErr error
	...
	for attempt := 1; attempt <= rc.retryConfig.MaxAttempts; attempt++ {
		...
		response, err := rc.circuitBreaker.Execute(ctx, func() (*AnalysisResponse, error) {
		...
		if analysisErr, ok := err.(*AnalysisError); ok && !analysisErr.Retryable {
