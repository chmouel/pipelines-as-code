...
const (
	...
	// FixActionIdentifier is the GitHub check-run requested_action identifier for the Fix it button.
	FixActionIdentifier = externalIDKindFix
...
func fixPipelineRunName(parentName, roleName string) string {
	return hashedPipelineRunName(parentName, roleName, "fix", fmt.Sprintf("%s-%s-fix", parentName, roleName))
}
