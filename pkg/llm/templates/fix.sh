...
# Use human author only if we have both name and email
...
# Validate and decode the stored patch
if [ -z "${PATCH_DATA_B64GZ}" ]; then
  echo "fix: PATCH_DATA_B64GZ is empty" >&2
  jq -n \
    '{status: "error", backend: "patch-apply", model: "", duration_ms: 0, error: {provider: "patch-apply", type: "empty_patch", message: "Patch data is empty. The analysis may not have produced a usable diff. Please re-run analysis.", retryable: false}}' >/tmp/fix-envelope.json
  emit_envelope /tmp/fix-envelope.json
  exit 1
fi
