...
func (p *PacRun) getPipelineRunsFromRepo(ctx context.Context, repo *v1alpha1.Repository) ([]matcher.Match, error) {
	...
	if p.event.TargetTestPipelineRun == "" {
		...
		p.logger.Debug("pre-matching pipelineruns for dynamic annotations, skipping CEL validation reporting")
		_, _ = matcher.MatchPipelinerunByAnnotation(ctx, p.logger, rtypes.PipelineRuns, p.run, p.event, p.vcx, p.eventEmitter, repo, &matcher.MatchOptions{
			SkipCELValidationErrorReport: true,
		})
	}
