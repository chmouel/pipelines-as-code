...
func (r *Reconciler) handleCheckRunOutput(ctx context.Context, logger *zap.SugaredLogger, result llm.AnalysisResult, event *info.Event, provider provider.Interface) error {
	...
	if err := provider.CreateStatus(ctx, event, status); err != nil {
...
func (r *Reconciler) handleAnnotationOutput(ctx context.Context, logger *zap.SugaredLogger, result llm.AnalysisResult, event *info.Event) error {
