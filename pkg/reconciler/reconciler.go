...
func (r *Reconciler) performLLMAnalysis(ctx context.Context, logger *zap.SugaredLogger, repo *v1alpha1.Repository, pr *tektonv1.PipelineRun, event *info.Event, provider provider.Interface) error {
	...
	// Perform analysis
	result, err := llm.AnalyzeFailure(ctx, r.run, r.kinteract, logger, pr, event, repo.Spec.Settings.AIAnalysis, repo.Namespace)
	if err != nil {
		return fmt.Errorf("LLM analysis failed: %w", err)
	}

	// If no result (e.g., pipeline didn't fail and analysis is failure-only), skip
	if result == "" {
		logger.Debug("No analysis result generated")
		return nil
	}

	// Post result as PR comment
	if event != nil && event.PullRequestNumber > 0 {
		comment := fmt.Sprintf("## 🤖 AI Analysis\n\n%s\n\n---\n*Generated by Pipelines-as-Code*", result)
		if err := provider.CreateComment(ctx, event, comment, "llm-analysis"); err != nil {
			return fmt.Errorf("failed to create PR comment: %w", err)
		}
		logger.Infof("Posted LLM analysis as PR comment for pipeline %s/%s", pr.Namespace, pr.Name)
	}
