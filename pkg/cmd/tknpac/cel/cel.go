...
func detectProvider(headers map[string]string, body []byte) (string, error) {
	...
	if headers["X-GitHub-Event"] != "" || headers["x-github-event"] != "" {
		...
		if headers["X-Gitea-Event-Type"] != "" || headers["x-gitea-event-type"] != "" {
			return "gitea", nil
		}
		return "github", nil
	}
	...
	if headers["X-Gitlab-Event"] != "" || headers["x-gitlab-event"] != "" {
		return "gitlab", nil
	}
...
func eventFromGitLab(body []byte, headers map[string]string) (*info.Event, error) {
	event := info.NewEvent()
	event.EventType = headers["X-Gitlab-Event"]
	if event.EventType == "" {
		event.EventType = headers["x-gitlab-event"]
	}
	event.Request.Payload = body
...
func eventFromGitea(body []byte, headers map[string]string) (*info.Event, error) {
	event := info.NewEvent()
	event.EventType = headers["X-Gitea-Event-Type"]
	if event.EventType == "" {
		event.EventType = headers["x-gitea-event-type"]
	}
	if event.EventType == "" {
			...
			if term.IsTerminal(int(os.Stdin.Fd())) {
				...
				if err != nil {
					...
					for {
						...
						// Create files data structure (empty for now since we don't have a provider)
