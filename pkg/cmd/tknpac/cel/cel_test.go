...
func TestEventFromGitHub(t *testing.T) {
  ...
  "number": 2126,
  "pull_request": {
    "title": "fix(bitbucketdatacenter): ignore branch deletion events in ParsePayload",
    "number": 2126,
    "user": {
      "login": "infernus01"
    },
    "head": {
      ...
      "repo": {
        "html_url": "https://github.com/infernus01/pipelines-as-code"
      }
    },
    "base": {
      "ref": "main",
	...
	tests := []struct {
	...
	}{
		{
			...
			checks: func(t *testing.T, event *info.Event) {
				...
				assert.Equal(t, event.Sender, "infernus01")
				assert.Equal(t, event.PullRequestTitle, "fix(bitbucketdatacenter): ignore branch deletion events in ParsePayload")
				assert.Equal(t, event.PullRequestNumber, 2126)
...
func TestCommandFlags(t *testing.T) {
	...
	assert.Equal(t, headersFlag.Usage, "path to JSON headers file")
...
func TestCELExpressionEvaluation(t *testing.T) {
  ...
  "sender": {
	...
	tests := []struct {
		...
		{
			name:       "pac sender",
			expression: `pac.sender == "infernus01"`,
			wantResult: "true",
		},
		{
			name:       "body action",
