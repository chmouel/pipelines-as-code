...
├── e2e-test-output.json          # Go test JSON event stream, when available
├── e2e-test-output.log           # Plain test execution output, when available
...
**Format**: JSON objects, usually one object per line from `go test -json`
**Content**:

- test start, output, pass, fail, and skip events
- failed test names
- assertion output, panic output, and timeout text
...
**Format**: Plain text, sometimes mixed with JSON-like snippets
**Size**: ~500KB
**Content**:
